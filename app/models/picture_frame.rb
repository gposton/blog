class PictureFrame
  ...
  def self.replace_displayed_with(new_photo)
    ...
    %x{ssh -q -o 'ConnectTimeout 2' gposton@roush 'pkill chrome'}
    %x{ssh -q -o 'ConnectTimeout 2' -X gposton@roush 'DISPLAY=:0.0 nohup google-chrome --kiosk http://www.glennposton.com/picture_frame/show?layout=false > /dev/null 2> /dev/null &'}
  end
end
