...
namespace :deploy do
 ...
 task :restart, :roles => :app, :except => { :no_release => true } do
   ...
   run "cd #{release_path} && bundle exec mongrel_rails start -p 8000"
   run "cd #{release_path} && bundle exec mongrel_rails start -p 8001"
   run "cd #{release_path} && bundle exec mongrel_rails start -p 8002"
 end
